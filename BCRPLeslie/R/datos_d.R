...
######################################
datos_d<-function(codigo, fecha.i,fecha.f){
  base="https://estadisticas.bcrp.gob.pe/estadisticas/series/diarias/resultados/"
  fecha=paste0(fecha.i[1],'-',fecha.i[2],'-',fecha.i[3],'/',fecha.f[1],'-',fecha.f[2],'-',fecha.f[3])
  ...
  a<-na.omit(a)
  index2<-a
  index2<-tidyr::extract(index2,fechas, into=c('d','m','y'), "(.{2})(.{3})(.{2})")
  index2<-index2 %>%
    dplyr::mutate(m=dplyr::recode(m,'Ene'='01','Feb'='02','Mar'='03','Abr'='04',
                    'May'='05','Jun'='06','Jul'='07','Ago'='08',
                    'Set'='09','Oct'='10','Nov'='11','Dic'='12'))
  index2$fecha<-apply(cbind(index2[,c(1,2,3)]),1,paste0, collapse="/")
  index2<-index2[,!(names(index2) %in% c('d','m','y'))]
  index2$fecha<-zoo::as.Date(index2$fecha,format="%d/%m/%y")
  index2$x<-as.numeric(index2$x)
  a<-index2
